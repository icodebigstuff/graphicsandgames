                                          ...
                                          const Matrix4 &view_matrix, const Matrix4 &proj_matrix)
{
  ...
  //  quick_shapes_.DrawAxes(ctm * S, view_matrix, proj_matrix);
    ...
    if (bone_name == "lowerback") {
        quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, Color(0, 0, 0), Point3(0, 0, 0), Point3(0, 0, 0) + skeleton_.BoneDirectionAndLength(bone_name), 0.01);
        quick_shapes_.DrawSphere(ctm * Matrix4::RotationX(-45) * Matrix4::Scale(Vector3(0.11, 0.11, 0.3)) * Matrix4::Translation(Vector3(0, 0,-0.5)), view_matrix, proj_matrix, Color(0.5, 0, 0));
    }
    if (bone_name == "upperback") {
        quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, Color(0, 0, 0), Point3(0, 0, 0), Point3(0, 0, 0) + skeleton_.BoneDirectionAndLength(bone_name), 0.01);
        quick_shapes_.DrawSphere(ctm * Matrix4::Scale(Vector3(0.1, 0.1, 0.1)) * Matrix4::Translation(Vector3(0, 0.1, 0)), view_matrix, proj_matrix, Color(0.5,0,0));
    }
    
    if (bone_name == "thorax") {
        quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, Color(0, 0, 0), Point3(0, 0, 0), Point3(0, 0, 0) + skeleton_.BoneDirectionAndLength(bone_name), 0.01);
        quick_shapes_.DrawSphere(ctm * Matrix4::Scale(Vector3(0.1, 0.1, 0.1)) * Matrix4::Translation(Vector3(0, 0.75, 0)), view_matrix, proj_matrix, Color(0.5, 0, 0));
    }
    ...
    if (bone_name == "head") {
        ...
        quick_shapes_.DrawSphere(ctm * Matrix4::RotationX(-45) * Matrix4::Scale(Vector3(0.1, 0.2, 0.1)) * Matrix4::Translation(Vector3(0, -0.4, 0.5)), view_matrix, proj_matrix, Color(0.5, 0, 0));

        quick_shapes_.DrawSphere(ctm * Matrix4::Scale(Vector3(0.05, 0.05, 0.04)) * Matrix4::Translation(Vector3(-0.5, 1, 3)), view_matrix, proj_matrix, Color(0, 0, 0));

        quick_shapes_.DrawSphere(ctm * Matrix4::Scale(Vector3(0.05, 0.05, 0.04)) * Matrix4::Translation(Vector3(0.5, 1, 3)), view_matrix, proj_matrix, Color(0, 0, 0));

        quick_shapes_.DrawSphere(ctm * Matrix4::Scale(Vector3(0.05, 0.02, 0.041)) * Matrix4::Translation(Vector3(0, -3.2, 6)), view_matrix, proj_matrix, Color(0, 0, 0));


        quick_shapes_.DrawCone(ctm * Matrix4::RotationZ(-50) * Matrix4::Scale(Vector3(0.02, 0.09, 0.01)) * Matrix4::Translation(Vector3(-2, 2, -3)), view_matrix, proj_matrix, Color(0, 0, 0));
        quick_shapes_.DrawCone(ctm *Matrix4::Translation(Vector3(0.1, 0.2, 0.04))* Matrix4::RotationX(90) * Matrix4::Scale(Vector3(0.02, 0.09, 0.02)) ,view_matrix, proj_matrix, Color(0, 0, 0));
