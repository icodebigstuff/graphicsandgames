...
void QuakeApp::DrawUsingOpenGL() {
    ...
    //earthquake magnitude sizes references from: https://en.wikipedia.org/wiki/Richter_magnitude_scale (richter magnitude scale)
    /*Great	8 or more
      Major	7 - 7.9
      Strong	6 - 6.9
      Moderate	5 - 5.9
      Light	- less then 5 (bit different but wanted to not be too complicated, everything below moderate will be light */
    double year = current_time_ - PLAYBACK_WINDOW;
    int start = quake_db_.FindMostRecentQuake(year); //start from a year ago
    int end = quake_db_.FindMostRecentQuake(current_time_); //end at current time
    double size;
    Color color;
    double min = quake_db_.min_magnitude();
    double max = quake_db_.max_magnitude();
    Point3 t = Point3(0,0,0);
    for (int i = start; i < end; i++) {  

      if (earth_.getGlobe() == true) { 
            t = earth_.LatLongToSphere(GfxMath::ToRadians(quake_db_.earthquake(i).latitude()),GfxMath::ToRadians(quake_db_.earthquake(i).longitude()));
        }
        else { 
            t = earth_.LatLongToPlane(GfxMath::ToRadians(quake_db_.earthquake(i).latitude()), GfxMath::ToRadians(quake_db_.earthquake(i).longitude()));
        } 

        double magnitude = quake_db_.earthquake(i).magnitude();
        double latitude = quake_db_.earthquake(i).latitude();
        double longitude = quake_db_.earthquake(i).longitude();
        size = (magnitude - min) / (max - min);
        if (magnitude >= 8) { //great
            color = Color(1, 0, 0); //red
        }
        else if (magnitude < 8 && magnitude >= 7) { //major
            color = Color(1, 0.647, 0); //orange
        }
        else if (magnitude < 7 && magnitude >= 6) { //strong
            color = Color(1, 1, 0); //yellow
        }
        else if (magnitude < 6 && magnitude >= 5) { //moderate
            color = Color(0, 1, 0); //green
        }
        else { //light
            color = Color(0, 0, 1); //blue
        }
       // Point3 flat = earth_.LatLongToPlane(GfxMath::ToRadians(latitude), GfxMath::ToRadians(longitude));
        Matrix4 quakes = Matrix4::Translation(Vector3(t[0],t[1], t[2])) * Matrix4::Scale(Vector3(size/7, size/7, size/7));


       
        quick_shapes_.DrawSphere(model_matrix * quakes, view_matrix_, proj_matrix_, color); //sphere
    }
