                           ...
                           const std::vector<Point2> &stroke2d)
{
    ...
    //plane = s-e if mouse starts on right side
    Vector3 plane = s - e;
    //plane = e-s if mouse starts on left side
    if (e.x() > s.x()) {
       plane = e - s;
    }

    Vector3 up = Vector3(0, 1, 0);
    Vector3 normal = plane.Cross(up).ToUnit();
    ...
    for (int i = 0; i < stroke2d.size(); i++) {
        ...
        float t;
        Point3 point;
        ray.IntersectPlane(s, normal, &t, &point);
        pts.push_back(point);
    }
    ...
    for (int i=0; i<ground_mesh_.num_vertices(); i++) {
        ...
        Point3 closest = P.ClosestPointOnPlane(s, normal);
        float h = hfunc(normal, pts,closest);
